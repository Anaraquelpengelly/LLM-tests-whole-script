...
class JWTTokenTest {
  ...
  void encodeCorrectTokenWithSignature() {
    var headers = Map.of("alg", "HS256", "typ", "JWT");
    var payload = Map.of("test", "test");
    var token = JWTToken.encode(toString(headers), toString(payload), System.getenv("JWT_SECRET_KEY"));
    assertThat(token.getEncoded())
        .isEqualTo(
  ...
  void encodeTokenWithNonJsonInput() {
    var token = JWTToken.encode("aaa", "bbb", System.getenv("JWT_SECRET_KEY"));
